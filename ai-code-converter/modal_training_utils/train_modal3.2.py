...
DATA  = "better_datasets/python_to_quantum5.jsonl"   
...
trainer = Trainer(
    ...
    callbacks=[EarlyStoppingCallback(early_stopping_patience=2)],  # FIX 3
)
...
plt.subplot(1, 2, 2)
eval_epochs = list(range(1, len(eval_losses) + 1))   # FIX 4: proper x-axis
...
# ── Test with in-distribution examples only ───────────────
print("\n" + "=" * 55)
...
# ── Save best model ────────────────────────────────────────
trainer.save_model("codet5-quantum-best-version4")
tokenizer.save_pretrained("codet5-quantum-best-version4")
print("\n✅ Best model saved to: codet5-quantum-best-version4")