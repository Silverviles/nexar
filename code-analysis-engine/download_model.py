...
Environment variables:
    GCS_MODEL_URI   — GCS path to the model directory (e.g. gs://nexar_models/code-analysis-engine/trained)
    ML_MODELS_DIR   — Local directory to download the model into (default: models/trained)
...
def download_model_from_gcs(gcs_uri: str, local_dir: str) -> None:
    ...
    Args:
        gcs_uri:   A gs:// URI, e.g. gs://nexar_models/code-analysis-engine/trained
...
def main() -> None:
    ...
    models_dir = os.getenv("ML_MODELS_DIR", "models/trained")

    if not gcs_uri:
        logger.info("GCS_MODEL_URI is not set — skipping model download.")
        return

    # If the model directory already has .pkl files, skip download
    if os.path.isdir(models_dir):
        pkl_files = [f for f in os.listdir(models_dir) if f.endswith(".pkl")]
        if pkl_files:
            logger.info(
                "Model directory %s already has %d .pkl file(s) — skipping download.",
                models_dir,
                len(pkl_files),
            )
            return
    ...
    try:
        download_model_from_gcs(gcs_uri, models_dir)
    except Exception:
