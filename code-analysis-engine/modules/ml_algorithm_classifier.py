...
class MLAlgorithmClassifier:
    ...
    def __init__(self, models_dir: str = os.getenv("ML_MODELS_DIR", "models/trained")):
    ...
    def extract_features_from_ast(
        ...
        avg_gates_per_qubit = quantum_metrics.gate_count / max(
            quantum_metrics.qubits_required, 1
        )
        ...
        # Ratio metrics
        single_qubit_ratio = quantum_metrics.single_qubit_gates / max(
            quantum_metrics.gate_count, 1
        )
        two_qubit_ratio = quantum_metrics.two_qubit_gates / max(
            quantum_metrics.gate_count, 1
        )
        ...
        # Depth-to-gate ratio
        depth_to_gate_ratio = quantum_metrics.circuit_depth / max(
            quantum_metrics.gate_count, 1
        )
    ...
    def classify(
        self, unified_ast: UnifiedAST, quantum_metrics, use_ensemble: bool = True
    ) -> Dict:
        """
        ...
        try:
            ...
            if use_ensemble:
                ...
                confidence = float(
                    np.max(self.random_forest.predict_proba(features_scaled)[0])
                )
