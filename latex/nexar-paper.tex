...
\author{
\IEEEauthorblockN{1\textsuperscript{st} Siriwardhana A H L T S}
...
\and
\IEEEauthorblockN{2\textsuperscript{nd} Prasad H G A T}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{and Software Engineering} \\
\textit{Sri Lanka Institute of Information} \\
\textit{Technology} \\
...
hgp.ashi@gmail.com}
\and
\IEEEauthorblockN{3\textsuperscript{rd} Jayasinghe Y L}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{and Software Engineering} \\
\textit{Sri Lanka Institute of Information} \\
\textit{Technology} \\
...
\and
\IEEEauthorblockN{4\textsuperscript{th} Hettiarachchi S R}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{and Software Engineering} \\
\textit{Sri Lanka Institute of Information} \\
\textit{Technology} \\
...
\and
\IEEEauthorblockN{5\textsuperscript{th} Nuwan Kodagoda}
...
nuwan.k@sliit.lk}
\and
\IEEEauthorblockN{6\textsuperscript{th} Kapila Dissanayaka}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{and Software Engineering} \\
\textit{Sri Lanka Institute of Information} \\
\textit{Technology} \\
...
Nexar is a six-service microservices monorepo coordinated by a central API Gateway (Fig.~\ref{fig:system-architecture}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{system-architecture.png}
\caption{Nexar architecture: source code flows from Frontend to Code Analysis, Decision Engine, AI Code Converter, and Hardware Abstraction Layer through the API Gateway.}
\label{fig:system-architecture}
\end{figure}

Submitted code enters a JWT-authenticated Express.js API Gateway, then the Code Analysis Engine extracts language, complexity, and algorithm signals. The Decision Engine recommends a target backend; when classical code is routed to quantum execution, the AI Code Converter generates Qiskit before the Hardware Abstraction Layer dispatches the job. Services are containerized with Docker and deployed on Google Cloud Run via Terraform.
...
The Hardware Abstraction Layer provides a provider-extensible interface for quantum job submission. IBM Qiskit is the fully implemented backend used for all real-hardware results in Table~\ref{tab:hardware-validation}; AWS Braket and Azure Quantum provider classes implement the same signature as architectural scaffolding but their job-execution methods are stubs in the present release (Section~\ref{sec:future-work}). The IBM implementation manages the full job lifecycle (device discovery, submission with target device/shots/transpilation options, status tracking for queued/running/completed/failed, and result retrieval with measurement distributions/metrics/metadata) through a factory-based abstraction that isolates routing from provider-specific SDKs, allowing new providers to be added by implementing a single interface.

\section{Experimental Evaluation}

\subsection{Benchmark Design}

We evaluated Nexar on 100 workloads in three categories: \emph{Classical} (32; deep learning, scientific computing, ML, cryptography, and combinatorial optimization), \emph{Hybrid} (23; VQE, QAOA, quantum ML, and ZNE), and \emph{Quantum} (45; 1--150q molecular simulation \cite{kandala2017}, Shor factoring, Grover search, QAOA optimization, and foundational circuits DJ/BV/Simon/QFT/QPE).
...
Because routing can be judged by different criteria, we evaluate against two independent oracles derived from workload metadata and physical circuit features (qubits, depth, entanglement), without using Nexar outputs. Oracle~A is fully independent of Nexar logic. Oracle~B uses externally sourced NISQ-viability thresholds (IBM specs + prior error-budget analysis), sharing first-principles physics with the rule engine but no internal co-calibration (Section~\ref{sec:discussion-effectiveness}).
...
Accuracy is higher under Oracle~B (0.86) than Oracle~A (0.81) because Nexar targets present-day hardware viability. Of 19 Nexar--Oracle~A disagreements, 18 appear in Table~\ref{tab:hardware-validation}: sub-20-qubit or deep circuits labeled Quantum by Oracle~A produced noise-dominated output (top outcome $<$10\%) on \texttt{ibm\_fez}, supporting Nexar's Classical routing.
...
With only 14/7 Quantum positives, single flips move F1$_{(Q)}$ by 0.07--0.14. Table~\ref{tab:statistical-significance} reports 95\% paired-bootstrap CIs (2{,}000 resamples) for F1$_{(Q)}$ and $\kappa$, plus McNemar tests versus Nexar (exact binomial for $b{+}c{<}25$, otherwise continuity-corrected $\chi^{2}$).
...
Aggregate metrics in Table~\ref{tab:routing-accuracy} mask Nexar's two-stage pipeline. The merger runs only on 39 \texttt{ALLOW\_BOTH} workloads; 61 are resolved by deterministic rule overrides. Table~\ref{tab:regime-conditional} reports both regimes separately.
...
\multicolumn{5}{l}{\footnotesize \texttt{ALLOW\_BOTH} 4 (A) / 2 (B). B1, B2, and A2 make zero quantum predictions there} \\
\multicolumn{5}{l}{\footnotesize by construction (thresholds not met; rules defer), so quantum-class recall is 0.} \\
\end{tabular}
\end{table}

On rule-overridden workloads, Nexar and rules-only are identical (F1$_{(Q)}$ 0.778/0.769) because stage one is the rule engine; B1 trails by 25 points on Oracle~A, indicating force-overrides add value beyond a single-feature threshold. In \texttt{ALLOW\_BOTH}, rules-only, B1, and B2 emit zero quantum predictions (recall 0) at sub-50-qubit scale, while the ensemble recovers 100\% of true-Quantum workloads under Oracle~B (ZNE 24q, QFT 20q) and 25\% under Oracle~A, with 11 false-positive quantum routes.

Within this slice, ML-only A1 slightly exceeds the full ensemble on F1$_{(Q)}$ (0.308 vs.\ 0.267 on B; 0.133 vs.\ 0.118 on A). Both have perfect recall; the gap is precision (two extra false positives: QPE and Simon), consistent with a classical bias from the 15\% cost weight in sub-120-qubit workloads. A practical improvement is regime-adaptive weighting (rules as gate, ML-dominant voting in \texttt{ALLOW\_BOTH}; Section~\ref{sec:future-work}).
...
For hardware validation, we submitted 27 circuits across all algorithm families to \texttt{ibm\_fez} (156-qubit Heron~r2) via Nexar's HAL at 1{,}024 shots each. Job IDs are listed in supplementary results and verifiable on the IBM Quantum Platform. Table~\ref{tab:hardware-validation} summarizes outcomes.
...
\bibitem{tang2024}
W.~Tang, Y.~Duan, Y.~Kharkov, R.~Fakoor, E.~Kessler, and Y.~Shi, ``AlphaRouter: Quantum circuit routing with reinforcement learning and tree search,'' arXiv:2410.05115, Oct. 2024. [Online]. Available: \url{https://arxiv.org/abs/2410.05115}
...
\bibitem{wang2021}
Y.~Wang, W.~Wang, S.~Joty, and S.~C.~H.~Hoi, ``CodeT5: Identifier-aware unified pre-trained encoder-decoder models for code understanding and generation,'' arXiv:2109.00859, Sep. 2021. [Online]. Available: \url{https://arxiv.org/abs/2109.00859}
