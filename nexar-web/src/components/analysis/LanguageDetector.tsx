...
import { getLanguageDisplayName, getLanguageColor } from '@/lib/languageDetection'
...
export function LanguageDetector({ code, className }: LanguageDetectorProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        setIsScanning(true)
        const result = await codeAnalysisAPI.detectLanguage({ code: trimmedCode })
        if (!canceled) {
          setDetection(result)
        }
      } catch (error) {
        if (!canceled) {
          setDetection({
            ...
            confidence: 0,
            is_supported: false,
            details: error instanceof Error ? error.message : 'Language detection unavailable',
            method: 'error',
          })
        }
      } finally {
    ...
    <div className={cn('flex items-center gap-2', className)}>
      {isScanning ? (
        <>
          ...
          <span className="text-sm text-ink-muted">Scanning patterns...</span>
        </>
      ) : language !== 'unknown' ? (
        <>
          <CheckCircle className="h-4 w-4 text-success" />
          <Badge variant="outline" className={cn('gap-1.5 border-primary/30 bg-primary/5', getLanguageColor(language))}>
            <Code2 className="h-3 w-3" />
            {getLanguageDisplayName(language)}
            <span className="text-xs opacity-70">{(confidence * 100).toFixed(0)}%</span>
            <span className="text-[10px] uppercase tracking-wide opacity-70">{method}</span>
          </Badge>
        </>
      ) : (
        <>
          <Code2 className="h-4 w-4 text-ink-muted" />
          <span className="text-sm text-ink-muted">Language not detected</span>
        </>
      )}
    </div>
  )
