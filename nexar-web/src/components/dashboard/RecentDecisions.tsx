...
export function RecentDecisions({ decisions, isLoading }: RecentDecisionsProps) {
  return (
          ...
          <div className="space-y-2">
            {decisions.map((decision) => {
              ...
              return (
                <div
                  key={decision.id}
                  className="flex flex-col gap-3 border border-hairline bg-surface-1 p-3 transition-colors hover:border-primary/40 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    <StatusIcon className={cn('h-4 w-4 sm:h-5 sm:w-5', statusColor[decision.status] || 'text-ink-muted')} />
                    <div>
                      <p className="font-mono text-xs font-medium sm:text-sm text-ink">{decision.id.substring(0, 8)}...</p>
                      <p className="text-xs text-ink-subtle">{timeAgo(decision.createdAt)}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between gap-2 sm:gap-4">
                    ...
                    <div className="text-right">
                      <p className="font-mono text-xs font-semibold sm:text-sm text-ink">{decision.confidence}%</p>
                      <p className="text-xs text-ink-subtle">confidence</p>
                    </div>
                    <p className="w-12 text-right font-mono text-xs text-ink-muted sm:w-16 sm:text-sm">
