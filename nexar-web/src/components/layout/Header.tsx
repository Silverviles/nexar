  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-hairline bg-canvas px-4 md:px-6">
      <div className="ml-12 md:ml-0">
        <h1 className="text-lg font-normal text-ink md:text-xl">{title}</h1>
        {description && <p className="hidden text-sm text-ink-muted sm:block">{description}</p>}
      </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="gap-2 px-2 md:px-4">
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-surface-1 md:h-7 md:w-7">
