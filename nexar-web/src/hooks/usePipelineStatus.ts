...
export function usePipelineStatus({ pipelineId, enabled = true }: UsePipelineStatusOptions): PipelineStatusState {
  ...
  const poll = useCallback(async () => {
    try {
      ...
      setState((prev) => ({
        ...prev,
        pollError: error instanceof Error ? error.message : 'Failed to fetch pipeline status',
      }))
    }
