...
export default function Pipeline() {
  ...
  return (
    <MainLayout title="Run Pipeline" description="Analyze code and get hardware recommendations in one step">
      <div className="space-y-6">
        <Card variant="glass" className="p-4">
          <PipelineStepper currentStage={stage} />
        </Card>

        <PipelineCodeInput code={code} onCodeChange={setCode} onSubmit={handleRunPipeline} isRunning={isRunning || isSubmitting} />
        ...
        {isRunning && (
          <FadeIn>
            <Card variant="glass">
              ...
              <CardContent>
                <Progress value={stage === 'analyzing' ? 40 : stage === 'deciding' ? 75 : 20} className="h-2" />
        ...
        {pipelineStatus.pollError && !error && (
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>Connection issue: {pipelineStatus.pollError}. Retrying...</AlertDescription>
          </Alert>
        )}

        {analysisResult && <PipelineAnalysisResults result={analysisResult} />}

        {decisionResult && !isRunning && <PipelineDecisionResults result={decisionResult} mappedInput={mappedInput} />}

        {decisionResult?.recommendation && stage === 'complete' && (
          <>
            <Separator />
            <PipelineExecution recommendedHardware={decisionResult.recommendation.recommended_hardware} code={code} />
          </>
        )}

        {pipelineStatus.timing && !isRunning && (
          <div className="flex items-center gap-4 text-xs text-ink-muted">
            {pipelineStatus.timing.analysis_ms != null && <span>Analysis: {pipelineStatus.timing.analysis_ms}ms</span>}
            {pipelineStatus.timing.decision_ms != null && <span>Decision: {pipelineStatus.timing.decision_ms}ms</span>}
            {pipelineStatus.timing.total_ms != null && <span>Total: {pipelineStatus.timing.total_ms}ms</span>}
          </div>
        )}

        {(stage === 'complete' || stage === 'error') && (
          <div className="flex justify-center">
            <button onClick={handleReset} className="text-sm text-ink-muted hover:text-ink transition-colors underline">
