...
export async function registerUser(email: string, password: string, name: string): Promise<RegisterResponse> {
  const { data } = await api.post<RegisterResponse>(`${AUTH_BASE}/register`, { email, password, name })
  return data
}

export async function loginUser(email: string, password: string): Promise<LoginResponse> {
  const { data } = await api.post<LoginResponse>(`${AUTH_BASE}/login`, { email, password })
  return data
}

export async function googleAuth(code: string): Promise<LoginResponse> {
  const { data } = await api.post<LoginResponse>(`${AUTH_BASE}/google`, { code })
  return data
}

export async function resendVerification(email: string): Promise<{ message: string }> {
  const { data } = await api.post<{ message: string }>(`${AUTH_BASE}/resend-verification`, { email })
  return data
}
