...
class CodeAnalysisAPI {
  ...
  private languageCache = new Map<string, { data: LanguageDetectionResponse; expiresAt: number }>()
  private inFlightLanguageRequests = new Map<string, Promise<LanguageDetectionResponse>>()
  ...
  private setLanguageCache(key: string, value: LanguageDetectionResponse): void {
    if (this.languageCache.size >= CodeAnalysisAPI.LANGUAGE_CACHE_MAX_ENTRIES) {
      const firstKey = this.languageCache.keys().next().value as string | undefined
      if (firstKey) {
   ...
   */
  async detectLanguage(submission: Pick<CodeSubmission, 'code'>): Promise<LanguageDetectionResponse> {
