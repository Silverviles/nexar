...
export const decisionEngineService = {
  ...
  async getHistory(params?: {
    ...
    const { data } = await api.get<DecisionHistoryResponse>(`${API_BASE}/history`, { params })
    return data
  },

  /** Get a single decision by ID */
  async getDecisionById(id: string): Promise<DecisionLogEntry> {
    const { data } = await api.get<DecisionLogEntry>(`${API_BASE}/history/${id}`)
    return data
  },

  /** Submit execution feedback for a decision */
  async submitFeedback(decisionId: string, feedback: FeedbackInput): Promise<{ success: boolean; message: string }> {
    const { data } = await api.post(`${API_BASE}/feedback/${decisionId}`, feedback)
    return data
  },
...
export const fieldDescriptions = {
  ...
  gate_count: 'Total number of quantum gates in your circuit',
