...
export const hardwareService = {
  ...
  async listProviderDevices(provider: string): Promise<HardwareDevicesResponse> {
    const { data } = await api.get<HardwareDevicesResponse>(`${API_BASE}/quantum/${provider}/devices`)
    return data
  },

  /** Submit a QASM circuit to a quantum provider for execution. */
  async submitQuantumCircuit(provider: string, request: QuantumExecuteRequest): Promise<JobSubmissionResponse> {
  ...
  /** Submit raw Python code for execution on IBM Quantum. Must define a `circuit` variable. */
  async submitPythonCode(request: PythonCodeExecuteRequest): Promise<JobSubmissionResponse> {
    const { data } = await api.post<JobSubmissionResponse>(`${API_BASE}/quantum/execute-python`, request)
    return data
  },

  /** Submit a classical compute task (Python code) to a classical provider. */
  async submitClassicalTask(provider: string, request: ClassicalExecuteRequest): Promise<JobSubmissionResponse> {
  ...
  async scheduleJob(request: ScheduleJobRequest): Promise<JobSubmissionResponse> {
    const { data } = await api.post<JobSubmissionResponse>(`${API_BASE}/quantum/schedule`, request)
    return data
  },

  /** List all scheduled jobs waiting for execution. */
  async getScheduledJobs(): Promise<ScheduledJobsResponse> {
    const { data } = await api.get<ScheduledJobsResponse>(`${API_BASE}/quantum/scheduled-jobs`)
    return data
  },

  /** Cancel a scheduled job before it executes. */
  async cancelScheduledJob(jobId: string): Promise<CancelJobResponse> {
    const { data } = await api.delete<CancelJobResponse>(`${API_BASE}/quantum/scheduled-jobs/${jobId}`)
    return data
  },
  ...
  async checkDeviceAvailability(device: string): Promise<DeviceAvailabilityResponse> {
    const { data } = await api.get<DeviceAvailabilityResponse>(`${API_BASE}/quantum/devices/${device}/availability`)
    return data
  },

  /** Get the current status of a job. */
  async getJobStatus(provider: string, jobId: string): Promise<JobStatusResponse> {
    const { data } = await api.get<JobStatusResponse>(`${API_BASE}/jobs/${provider}/${jobId}`)
    return data
  },

  /** Get the result of a completed job. */
  async getJobResult(provider: string, jobId: string): Promise<JobResultResponse> {
    const { data } = await api.get<JobResultResponse>(`${API_BASE}/jobs/${provider}/${jobId}/result`)
    return data
  },
}
